    ...
    <section id="container" >
        ...
        <section id="main-content">
            <section class="wrapper site-min-height">
                <div class="row">
                    <div class="col-lg-12">
                        <form class="cmxform form-horizontal tasi-form" id="addUserForm" onsubmit="return test()"  role="form" method="post"  action="<?php echo site_url('add_recent_news/uploadData');?>" enctype="multipart/form-data">
                            <div class="form-group">
                                <label for="inputSuccess" class="col-sm-2 control-label col-lg-2">Menu Name</label>
                                <div class="col-lg-10">
                                    <select class="form-control" name="select_menu" id="select_menu" onchange="getNewsContent(this.value)"> <!-- input-sm m-bot15  -->
                                        <option value="">Select Menu</option>
                                        <option value="1">News1</option>
                                        <option value="2">News2</option>
                                        <option value="3">News3</option>
                                        <option value="4">News4</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="inputSuccess" class="col-sm-2 control-label col-lg-2">Upload Title Image</label>
                                <div class="col-lg-10">
                                    <img src="" id="title_img">
                                    <input type="hidden" id="image_file_name" name="image_file_name" />
                                    <input type="file" name="userfileimg" id="userfileimg" size="20" />
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <label for="inputSuccess" class="col-sm-2 control-label col-lg-2">Upload File</label>
                                <div class="col-lg-8">
                                    <input type="text" id="file_title_name0" name="file_title_name0" />
                                    <input type="file" name="userfile0" id="userfile0" size="20" value=""/>

                                </div>
                                <div class="col-lg-2">
                                    <input type="text" id="total_counter_value" name="total_counter_value" value="0">
                                    <input type="button" class="form-control btn btn-default" onclick="getMoreRequisition();"   value="Add">
                                </div>
                            </div>
   ...
   <!--  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/js/bootstrap.min.js"></script>-->
    ...
    <script type="text/javascript">
        ...
        function getNewsContent(news_id){
            ...
            $.ajax({
                type: "Post",
                url: "<?php echo site_url('add_recent_news/getNewsContent');  ?>",
                data: {'news_id':news_id} ,
                success: function(data) {
                    var obj = jQuery.parseJSON(data);
                    var baseurl = "<?php echo base_url(); ?>";
                    var image_name=baseurl+"public/uploads/"+obj.image_name;
                    var content= obj.content;
                    var file_name= obj.file_name;

                    $("#title_img").attr("src",image_name);
                    $("#image_file_name").val(obj.image_name);
                    $('#txtEditor').data("editor").html(content);
                    $("#file_name").val(file_name);
                    // $( "#total_counter_value").val(0);
                }
            });
        }
        ...
        var counter=0
        function getMoreRequisition(){
            counter++;
            var html= '<div class="form-group" id="upload_div'+counter+'">'+
                '<label for="inputSuccess" class="col-sm-2 control-label col-lg-2">Upload File</label>'+
                '<div class="col-lg-8">'+
                '<input type="text" id="file_title_name'+counter+'" name="file_title_name'+counter+'" />'+
                '<input type="file" name="userfile'+counter+'" id="userfile'+counter+'" size="20" value=""/>'+
                '</div>'+
                '<div class="col-lg-2">'+
                '<input type="button" class="form-control btn btn-default" onclick="removeRequisition('+counter+');"   value="Remove">'+
                '</div>'+
                '</div>';
            $( "#total_counter_value").val(counter);
            $( "#requisition_div" ).append(html);
        }

        function removeRequisition(id){
